...
const EditUserForm: React.FC<EditUserFormProps> = ({ user }) => {
  ...
  async function onSubmit(data: EnteredData) {
    ...
    if (!isChecked) {
      alert(
        "Check if the form is filled in correctly! Maybe you forgot to tick the checkbox confirming that you are changing your details?"
      );
      return;
    }
      ...
      >
        ...
        <div className="pb-10">
          ...
          <label htmlFor="chooseCb" className="pl-2">
            I confirm that I am changing my data and not another users. Say no
            to pranks!
          </label>
          {errors.chooseCb && (
